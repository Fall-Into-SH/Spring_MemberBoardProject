...
<body>
	...
	<section id="hero" class="d-flex align-items-center">
		<div class="container position-relative text-center text-lg-start"
			...
			<div class="row">
				<c:if test="${loginId == modify.boWriter}">
					<form action="boModify" method="POST" enctype="multipart/form-data">
						<h1 class="text">게시글 수정</h1>

						<div class="field">
							<div id="board1">
								<select style="float: right;" name="boMode">
									<c:if test="${boardMode != 'all'}">
										<option value="${modify.boMode}">${modify.boMode}</option>
									</c:if>
								</select>
							...
							<table
								...
								<tbody>
									...
									<tr>
										<th>작성자</th>
										<td>${modify.boWriter}<input type="hidden" name="boNum"
											value="${modify.boNum}" /><input type="hidden" name="boMode1"
											value="${modify.boMode1}" /> <input type="hidden"
											name="boWriter" value="${modify.boWriter}" /> <input
											type="hidden" name="dFileName" value="${modify.DFileName}" /></td>
									</tr>

									<tr>
										<th>제목</th>
										<td><input type="text" name="boTitle"
											value="${modify.boTitle}"></td>
									</tr>
									<tr>
										<th>내용</th>
										<td style="height: 100px;"><textarea name="boContent">${modify.boContent}</textarea></td>
									</tr>
									<tr>
										<th>첨부파일</th>
...
function boardList(result){
	...
	output1+="<option value='${boardMode}'>${boardMode}</option>";
		...
		output1+="<option value='${modify.boMode}'>${modify.boMode}</option>";
	}
	if(${modify.boMode !='익명'}){
	for(let i in result){
		if(result[i].boMode!='익명'){
...
<c:choose>
	<c:when test="${loginId == modify.boWriter || loginId == 'admin'}">
