...
<body>
	...
	<form action="boardCreate" method="post" name="boardCreateForm">
		...
		<div class="member" style="color: black;">
			...
			<div class="field">
				<b style="color: white;">게시판 이름</b> <span><input type="text" name="boMode" id="modeName"
						 onkeyup="checkName()" placeholder="ex)입력: 자유 = 자유 게시판"><span id="check1" style="background-color: white"></span></span>
			</div>
			
			<!-- 6. 가입하기 버튼 -->
			<input style="background-color: orange; color: white" type="button" onclick="checkConfirm()" value="생성하기" >
...
function checkConfirm() {
	let modeName = $("#modeName").val();
	
	if(checkName()){
		
		let conf = confirm(modeName + ' 게시판을 생성하시겠습니까?');

		if (conf) {
			alert('생성했습니다.');
			boardCreateForm.submit();
		} else {
			alert('생성을 취소했습니다.');
		}
...
function checkName() {
	let modeName = $("#modeName").val();
	let spc = modeName.search(/\s/);
	let spe = modeName.search(/[`~!@#$%^&*|\\\'\":;\/?]/);
	...
	if(modeName==null || modeName==""){
		...
		alert('이름에 특수문자를 제거해 주세요');
	}else{

		$.ajax({
			type : "POST",
			url : "boardNameOverlap",
			data : {
				"boMode" : modeName
			},
			async : false,
			...
			success : function(result){
				if(result=="OK"){
					// 사용할 수 있는 게시판 이름일 경우 실행
					check1.innerHTML = "사용가능한 이름";
					check1.style.color = "#0000ff";
					conf = true;
				} else {
					// 사용할 수 없는 게시판 이름일 경우 실행
					check1.innerHTML = "이미 사용중인 이름";
					check1.style.color = "#ff0000";
				}
			},
			error : function(){
				alert("boardNameOverlap 함수 통신실패!");
			}
