...
<body>
	...
	<section id="hero" style="padding-top: 50px;">
		<div class="container position-relative text-center text-lg-start"
			...
			<div class="row">
				<h2 class="text">검색 목록</h2>
				<c:choose>
					<c:when test="${searchMode eq 'member'}">
						<table>
							<tr>
								<th>아이디</th>
								<th>이름</th>
							...
							<tr>
								<td colspan="5" class="trcenter">
									<!-- 페이징 처리 --> <!-- [처음]페이지에 대한 처리 --> <c:if
										test="${search.page <= 1}">
										<c:out value="<< 처음" />
									</c:if> <c:if test="${search.page > 1}">
										<a
											href="mSearch1?category=${search.category}&keyword=${search.keyword}&page=1&limit=${search.limit}"><c:out
												value="<< 처음" /></a>
									</c:if> <!-- [이전]페이지에 대한 처리 --> <c:if test="${search.page <= 1}">
										<c:out value="< 이전" />

									</c:if> <c:if test="${search.page > 1}">
										<a
											href="mSearch1?category=${search.category}&keyword=${search.keyword}&page=${search.page-1}&limit=${search.limit}"><c:out
												value="< 이전" /></a>
									</c:if> <!-- [페이지번호]페이지에 대한 처리 --> <c:forEach var="i"
										begin="${search.startPage}" end="${search.endPage}">

										<c:choose>

											<c:when test="${search.page == i}">
												<span class="space1">[${i}]</span>
											</c:when>

											<c:otherwise>
												<span class="space1"><a
													href="mSearch1?category=${search.category}&keyword=${search.keyword}&page=${i}&limit=${search.limit}">${i}
												</a></span>
											</c:otherwise>
										...
										test="${search.page >= search.maxPage}">
										<c:out value="다음 >" />
									</c:if> <c:if test="${search.page < search.maxPage}">
										<a
											href="mSearch1?category=${search.category}&keyword=${search.keyword}&page=${search.page+1}&limit=${search.limit}"><c:out
												value="다음 >" /></a>
										...
										<a
											href="mSearch1?category=${search.category}&keyword=${search.keyword}&page=${search.maxPage}&limit=${search.limit}"><c:out
												value="마지막 >>" /></a>
					...
					<c:when test="${searchMode eq 'board'}">
						<table>
							<thead>
								<tr>
									<th>번호</th>
							...
							<tbody>
								<c:forEach var="list" items="${boardList}">
									<c:if test="${list.boMode != '익명' || (loginId == list.boWriter && search.category != 'BOWRITER || BOTITLE')}">
										<tr>
											<td>${list.boNum}</td>
											<td><a href="boView?boNum=${list.boNum}">${list.boTitle}</a></td>
											<td>${list.boWriter}</td>
											<td>${list.boDate}</td>
											<td>${list.boHit}</td>
										</tr>
								...
								<tr>
									<td colspan="5" class="trcenter">
										<!-- 페이징 처리 --> <!-- [처음]페이지에 대한 처리 --> <c:if
											...
											<a
												href="boSearch1?category=${search.category}&keyword=${search.keyword}&page=1&limit=${search.limit}"><c:out
													value="<< 처음" /></a>
										</c:if> <!-- [이전]페이지에 대한 처리 --> <c:if test="${search.page <= 1}">
											<c:out value="< 이전" />
											...
											<a
												href="boSearch1?category=${search.category}&keyword=${search.keyword}&page=${search.page-1}&limit=${search.limit}"><c:out
													value="< 이전" /></a>
										</c:if> <!-- [페이지번호]페이지에 대한 처리 --> <c:forEach var="i"
											begin="${search.startPage}" end="${search.endPage}">
											...
											<c:choose>
												...
												<c:otherwise>
													<span class="space1"><a
														href="boSearch1?category=${search.category}&keyword=${search.keyword}&page=${i}&limit=${search.limit}">${i}
													</a></span>
												</c:otherwise>
											...
											<a
												href="boSearch1?category=${search.category}&keyword=${search.keyword}&page=${search.maxPage}&limit=${search.limit}"><c:out
													value="마지막 >>" /></a>
